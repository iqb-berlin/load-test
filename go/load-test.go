...
func makeRequest(method string, url string, payload string, authToken string, expectedStatus int) (*http.Response, error) {
	...
	for retries > 0 {
		...
		if errors.Is(err, syscall.ECONNRESET) {
			retries -= 1
			log.Printf("WARNING: " + url + " failed! Connection Reset; Retries left: " + strconv.Itoa(retries))
			if retries == 0 {
				err = errors.New("ERROR: " + url + " failed! Connection Reset; Retries left: " + strconv.Itoa(retries))
			}
			continue
		}
